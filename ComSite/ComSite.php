...
class ComSite extends plxPlugin {	 
	...
	public function plxShowLastComList($format, $max = 5, $art_id = '', $cat_ids = ''){ 
           if (!$format)  $format='<li><a href="#com_url">#com_author a dit :</a><p>#com_content(50) | Site web : <a href="#com_site">#com_site</a></p></li>';
    global $plxMotor;
       # Génération de notre motif
        ...
        $plxGlob_coms = clone $plxMotor->plxGlob_coms;
        if ($aFiles = $plxGlob_coms->query($motif, 'com', 'rsort', 0, false, 'before')) {
            ...
            foreach ($aFiles as $v) {
                ...
                if (isset($plxMotor->activeArts[substr($v, 0, 4)])) {
                    $com = $plxMotor->parseCommentaire(PLX_ROOT . $plxMotor->aConf['racine_commentaires'] . $v);
                    $artInfo = $plxMotor->artInfoFromFilename($plxMotor->plxGlob_arts->aFiles[$com['article']]);
                    if ($artInfo['artDate'] <= $datetime) { # on ne prends que les commentaires pour les articles publiés
                        if (empty($cat_ids) or preg_match('/(' . $cat_ids . ')/', $artInfo['catId'])) {
                            ...
                            $row = str_replace('#com_url', $plxMotor->urlRewrite($url), $row);
                            $row = str_replace('#com_author', $com['author'], $row);
			// ajout du champ pour extraire l'url du site 
			...
			// fin ajout
                            ...
                            if ($isComArtTitle) {
                                if (isset($aComArtTitles[$com['article']])) {
                                    ...
                                    if ($file = $plxMotor->plxGlob_arts->query('/^' . $com['article'] . '.(.*).xml$/')) {
                                        $art = $plxMotor->parseArticle(PLX_ROOT . $this->plxMotor->aConf['racine_articles'] . $file[0]);
      ...
      exit; // pour ne pas lancer la suite de la fonction native
       }
}
?>